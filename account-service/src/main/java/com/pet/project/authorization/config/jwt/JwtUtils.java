...
import com.pet.project.authorization.exception.TokenException;
...
public class JwtUtils {
    ...
    public boolean validateJwtToken(final String token) {
        ...
        if (jwt.getExpirationTime().compareTo(new Date()) < 0) {
            throw new TokenException(jwt.getSubject(), "Token was expired. Please make a new login request");
        }
        return true;
    }
    ...
    private ResponseCookie generateCookie(final String name, final String value, final String path) {
        return ResponseCookie.from(name, value).path(path).maxAge(this.jwtCookieAgeSec).httpOnly(true).build();
    }
